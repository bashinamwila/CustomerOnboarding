...
{
    [Serializable]
    public class FixedEmployerExpense : BusinessBase<FixedEmployerExpense>, IFixedEmployerExpense
    {
        ...
        [InsertChild]
        private void Insert(TenantOnboardingOrchestrator parent, [Inject] IFixedEmployerExpenseDal dal)
        {
            using (BypassPropertyChecks)
            {
        ...
        [FetchChild]
        private void Fetch(string tenantId, string id, string ruleSet, [Inject] IFixedEmployerExpenseDal dal)
        {
            using (BypassPropertyChecks)
            {
        ...
        [UpdateChild]
        private void Update(TenantOnboardingOrchestrator parent, [Inject] IFixedEmployerExpenseDal dal)
        {
            using (BypassPropertyChecks)
            {
                var amount = ReadProperty(AmountProperty);
        ...
        [DeleteSelfChild]
        private void Delete(TenantOnboardingOrchestrator parent, [Inject] IFixedEmployerExpenseDal dal)
        {
            var _EmployerExpense = (EmployerExpense)Parent;
            dal.Delete(parent.TenantId, _EmployerExpense.Id);
        }

        [DeleteSelfChild]
        private void Delete(string tenantId, string id, [Inject] IFixedEmployerExpenseDal dal)
        {
