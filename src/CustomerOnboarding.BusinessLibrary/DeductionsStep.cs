        ...
        StepBase<DeductionsStep>, ISkippable, IOnboardingOrchestrator
    {
            ...
            [Inject] IChildDataPortalFactory portal)
        {
            using (BypassPropertyChecks)
            {
                ...
                CurrentStepIndex = 0;
                IsSkipped = false;
                var _factory = await factory.GetPortal<DeductionsStepStepsFactory>().FetchAsync(new[] { 14, 15, 16 }, CurrentStepIndex);
        ...
        private async Task Insert(TenantOnboardingOrchestrator parent,
          [Inject] IDeductionsStepDal dal,
          [Inject] IChildDataPortal<Steps> portal)
        {
            using (BypassPropertyChecks)
            {
                var dto = new DeductionsStepDto
                {
            ...
            [Inject] IChildDataPortalFactory portal)
        {
            using (BypassPropertyChecks)
            {
                ...
                var _factory = await factory.GetPortal<DeductionsStepStepsFactory>().FetchAsync(tenantId, CurrentStepIndex);
                Steps = _factory.Steps;
