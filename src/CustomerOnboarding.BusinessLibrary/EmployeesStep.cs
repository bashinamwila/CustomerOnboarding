...
namespace CustomerOnboarding.BusinessLibrary
{
    ...
    [Serializable]
    public class EmployeesStep :
        StepBase<EmployeesStep>,IOnboardingOrchestrator
    {
            ...
            [Inject] IChildDataPortalFactory portal)
        {
            using (BypassPropertyChecks)
            {
                ...
                IsCompleted = false;
                CurrentStepIndex = 0;
                var _factory = await factory.GetPortal<EmployeesStepStepsFactory>().FetchAsync(new[] { 22 }, CurrentStepIndex);
        ...
        private async Task Insert(TenantOnboardingOrchestrator parent,
         [Inject] IEmployeesStepDal dal,
         [Inject] IChildDataPortal<Steps> portal)
        {
            using (BypassPropertyChecks)
            {
                var dto = new EmployeesStepDto
                {
            ...
            [Inject] IChildDataPortalFactory portal)
        {
            using (BypassPropertyChecks)
            {
                ...
                var _factory = await factory.GetPortal<EmployeesStepStepsFactory>().FetchAsync(tenantId, CurrentStepIndex);
                Steps = _factory.Steps;
