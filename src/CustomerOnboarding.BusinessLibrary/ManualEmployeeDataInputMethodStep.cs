...
{
    [Serializable]
    public class ManualEmployeeDataInputMethodStep :
        StepBase<ManualEmployeeDataInputMethodStep>,IEmployeeDataInputMethodStep,IOnboardingOrchestrator
    {
        ...
        protected override void OnChildChanged(ChildChangedEventArgs e)
        {
            ...
            if (e.ChildObject is IStep || e.ChildObject is Steps
              || e.ChildObject is EmployeeEmploymentDetails 
              || e.ChildObject is EmployeePersonalDetails)
          ...
          [Inject] IChildDataPortal<Steps> portal)
        {
            using (BypassPropertyChecks)
            {
                var dto = new ManualEmployeeDataInputMethodStepDto
                {
            ...
            [Inject] IChildDataPortalFactory portal)
        {
            using (BypassPropertyChecks)
            {
                ...
                var _factory = await factory.GetPortal<ManualEmployeeDataInputMethodStepStepsFactory>().FetchAsync(tenantId, CurrentStepIndex);
                Steps = _factory.Steps;
