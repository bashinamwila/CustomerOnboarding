...
namespace CustomerOnboarding.BusinessLibrary
{
    ...
    public class Wage : BusinessBase<Wage>, IEarning, IFormular
    {
        ...
        [RegularExpression("^1[0-9]{3}$", ErrorMessage = "The id of the wage should be between 1000 and 1999")]
        public string Id
        {
       ...
       // [Required(ErrorMessage = "Name is required")]
        ...
        [Display(Name = "Equals")]
       // [Required(ErrorMessage = "Formular is required")]
...
/*
        public static readonly PropertyInfo<Admin.Variables> VariablesProperty =
            RegisterProperty<Admin.Variables>(nameof(Variables));
        public Admin.Variables Variables
        {
...
*/
        protected override void AddBusinessRules()
        {
          ...
          //  BusinessRules.AddRule(new Csla.Rules.CommonRules.Dependency(FormularProperty, VariablesProperty));
           // BusinessRules.AddRule(
            ...
            BusinessRules.AddRule(new BusinessLibrary.Rules.Required(NameProperty) { MessageText = "Name is required" });
            BusinessRules.AddRule(new BusinessLibrary.Rules.Required(IdProperty) { MessageText = "Id is required" });
            BusinessRules.AddRule(new BusinessLibrary.Rules.Required(FormularProperty) { MessageText = "Formular is required" });
        ...
        [InsertChild]
        private void Insert(TenantOnboardingOrchestrator parent,[Inject] IWageDal dal)
        {
            using (BypassPropertyChecks)
            {
                var data = new WageDto
                {
                    ...
                    IsSystemDefined = this.IsSystemDefined!.Value,
        ...
        [FetchChild]
        private void Fetch(WageDto data,string ruleSet, [Inject] IWageDal dal)
        {
        ...
        [UpdateChild]
        private void Update(TenantOnboardingOrchestrator parent,[Inject] IWageDal dal)
        {
            using (BypassPropertyChecks)
            {
                var data = new WageDto
    ...
    /*
        [Delete]
        private async Task DeleteAsync(string id, [Inject] IWageDal dal,
            [Inject] IChildDataPortal<Variables> portal)
        {
            using (BypassPropertyChecks)
            {
                var variables = await portal.FetchChildAsync(id);
                variables.Clear();
