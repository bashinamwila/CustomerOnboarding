...
{
    [Serializable]
    public class PercentWageEmployerExpense : BusinessBase<PercentWageEmployerExpense>, IPercentWageEmployerExpense
    {
        ...
        [InsertChild]
        private void Insert(TenantOnboardingOrchestrator parent, [Inject] IPercentWageEmployerExpenseDal dal)
        {
            using (BypassPropertyChecks)
            {
        ...
        [FetchChild]
        private void Fetch(string tenantId, string id, string ruleSet, [Inject] IPercentWageEmployerExpenseDal dal)
        {
            using (BypassPropertyChecks)
            {
        ...
        [UpdateChild]
        private void Update(TenantOnboardingOrchestrator parent, [Inject] IPercentWageEmployerExpenseDal dal)
        {
            using (BypassPropertyChecks)
            {
                var item = new TypeOfEmployerExpenseDto
        ...
        [DeleteSelfChild]
        private void DeleteAsync(TenantOnboardingOrchestrator parent, [Inject] IPercentWageEmployerExpenseDal dal)
        {
            dal.Delete(parent.TenantId, ((EmployerExpense)Parent).Id);
        }

        [DeleteSelfChild]
        private void Delete(string tenantId, string id, [Inject] IPercentWageEmployerExpenseDal dal)
        {
